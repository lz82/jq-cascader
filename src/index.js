...
export default class Cascader {
  ...
  init() {
    ...
    this.list = $(
      `	<div class="ui-cascader ui-cascader-dropdown panel hid"></div>`
    );
    this.searchedList = $(
      `<div class="ui-cascader ui-cascader-dropdown searchedlist hid"></div>`
    );
    ...
    this.inputDom.bind("input", (e) => {
      ...
      if (searchKey) {
        if (!this.searchedList.parents("body").length) {
          $(document.body).append(this.searchedList);
          this.searchedList.delegate(
            "li.ui-cascader-menu-item",
            "click",
            (e) => {
